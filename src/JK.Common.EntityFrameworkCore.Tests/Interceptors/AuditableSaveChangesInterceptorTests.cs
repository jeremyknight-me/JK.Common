...
public class AuditableSaveChangesInterceptorTests
{
    ...
    public async Task SaveChangesAsync_Create()
    {
        ...
        {
            var entity = new SimpleAuditableEntity { Text = "Hello World" };
            _ = await context.SimpleEntities.AddAsync(entity, TestContext.Current.CancellationToken);
            auditedCount += await context.SaveChangesAsync(TestContext.Current.CancellationToken);
            entityId = entity.Id;
        }

        using (var context = this.MakeContext(databaseName))
        {
            var entity = await context.SimpleEntities.FirstOrDefaultAsync(x => x.Id == entityId, TestContext.Current.CancellationToken);
            Assert.Equal(1, auditedCount);
            Assert.Equal(entity.DateCreatedUtc, entity.DateModifiedUtc);
        }
    }
    ...
    public async Task SaveChangesAsync_Update()
    {
        ...
        using (var context = this.MakeContext(databaseName))
        {
            // create
            var entityToCreate = new SimpleAuditableEntity { Text = "Hello World" };
            _ = await context.SimpleEntities.AddAsync(entityToCreate, TestContext.Current.CancellationToken);
            auditedCount += await context.SaveChangesAsync(TestContext.Current.CancellationToken);
            entityId = entityToCreate.Id;

            // modify
            var entityToModify = await context.SimpleEntities.FirstOrDefaultAsync(x => x.Id == entityId, TestContext.Current.CancellationToken);
            entityToModify.Text = "Change Me!";
            auditedCount += await context.SaveChangesAsync(TestContext.Current.CancellationToken);
        }
